...
namespace
{
  ...
  static const bynav_oem7::version_element_t VERSION_MAJOR  = 1;
  static const bynav_oem7::version_element_t VERSION_MINOR  = 0;
  static const bynav_oem7::version_element_t VERSION_SPECIAL= 0;
}


namespace bynav_oem7
{
  /**
   * A wrapper for BaseMessageData
   * Hides EDIE accessors / data that need not be exposed (yet)
   */
  class OemRawMessage: public Oem7RawMessageIf
  {
    std::unique_ptr<BaseMessageData> bmd_; ///< binary message obtained from receiver


  public:
    OemRawMessage(BaseMessageData* raw_bmd):
      bmd_(raw_bmd)
    {
    }
...
public:
  ...
  virtual bool readMessage(std::shared_ptr<Oem7RawMessageIf>& msg)
  {
    ...
    if(raw_bmd)
    {
      msg = std::make_shared<OemRawMessage>(raw_bmd);
    }
